...
export default function Index() {
  ...
  const handleRightAction = async (id: string) => {
    ...
    const RepPlant = filteredPlant?.find((p) => (p.Plant_zone === SelectedPlant?.Plant_zone && p.plant_sno === 1)) || SelectedPlant
    const plantsInZone = filteredPlant?.filter((p) => p.Plant_zone === SelectedPlant?.Plant_zone) || [];
    try {
      if (!user || (wateredZone?.includes(Number(SelectedPlant?.Plant_zone)))) return;
      await triggerWatering(SelectedPlant?.Plant_zone, RepPlant.watering_duration_in_min, plantsInZone)
    } catch (error) {
      console.log(error)
    }
  ...
  useEffect(() => {
    // Clean up refs for plants that no longer exist
    const currentPlantIds = new Set(plant?.map(p => p.$id) || []);
  ...
  useEffect(() => {
    ...
    const see = () => {
      ...
      if (now - lastHeartbeat > 15000) {
        activate()
        console.log("switching down system")
      }
    }
    see();
    const interval = setInterval(see, 30000)
  ...
  const triggerWatering = async (zoneId: number, durationMinutes: number, plantsInZone: plant[]) => {
    try {
      const response = await fetch(`http://${Ip}:5000/water_zone`, {
        ...
        body: JSON.stringify({ // <-- The JSON data sent in the request body
          user_id: user?.$id,
          zone_id: zoneId,
                        ...
                        <View style={styles.cardHeader}>
                          <View>
                            ...
                            <Text style={styles.cardTitle}>
                              {p.Plant_name}
                              {p.plant_sno === 1 && <AntDesign name="star" size={24} color="orange" />}
                            </Text>
                          ...
                          <Image source={{ uri: p.url ? p.url : "https://bustaniplantfarm.com/wp-content/uploads/2022/06/plant_placeholder.png" }}
